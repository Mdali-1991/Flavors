...
/* ── PROTEIN-BASED PRICING (Noodles / Curries / Rice) ──
   Dishes in these categories share one menu but are priced by the
   protein the customer chooses. The dropdown rendered in buildMenu()
   lets the user switch protein at any time; every visible price and
   every "Add" action then reads from the option currently selected,
   rather than a price baked in when the page first loaded. */
function parsePrice(str) {
  return parseFloat(String(str).replace(/[^0-9.]/g, '')) || 0;
}

function getProteinPrice(cat, protein) {
  const table = MENU[cat] && MENU[cat].pricingTable;
  if (!table) return 0;
  const entry = table.find(p => p.protein === protein) || table[0];
...
function updateProteinPricing(cat, protein) {
  ...
  document.querySelectorAll(`#panel-${cat} [data-protein-price="${cat}"]`).forEach(el => {
    el.textContent = `£${price.toFixed(2)}`;
  });

  document.querySelectorAll(`#panel-${cat} [data-protein-item="true"]`).forEach(card => {
    const name = card.dataset.name;
    card.setAttribute('aria-label', `Add ${name} (${protein}) to cart for £${price.toFixed(2)}`);
  });

  showToast(`${MENU[cat].label} now priced for ${protein} — £${price.toFixed(2)}`);
}

function addProteinItemToCart(cardEl) {
  if (!cardEl) return;
  const cat = cardEl.dataset.cat;
  const name = cardEl.dataset.name;
  const select = document.getElementById(`protein-${cat}`);
  const protein = select ? select.value : MENU[cat].pricingTable[0].protein;
  const price = getProteinPrice(cat, protein);
...
function buildMenu() {
  ...
  Object.entries(MENU).forEach(([cat, data]) => {
    ...
    // Protein selector: an interactive dropdown replaces the old static
    // table so the customer can pick a protein and see prices for the
    // whole category update immediately, in place, before adding to cart.
    if (data.pricingTable) {
      const defaultProtein = data.pricingTable[0].protein;
      html += `<div class="protein-selector" role="group" aria-label="Choose a protein for ${data.label}">
        <label for="protein-${cat}">🍽 Choose your protein</label>
        <select id="protein-${cat}"
                class="protein-select"
                aria-describedby="protein-price-${cat}"
                onchange="updateProteinPricing('${cat}', this.value)">
          ${data.pricingTable.map((r, i) => `<option value="${r.protein}"${i === 0 ? ' selected' : ''}>${r.protein} — ${r.price}</option>`).join('')}
        </select>
        <span class="protein-current-price">Price: <strong id="protein-price-${cat}" aria-live="polite">${data.pricingTable[0].price}</strong></span>
      </div>`;
    }
    ...
    } else if (data.pricingTable) {
      // Items priced by the protein chosen in the dropdown above, rather
      // than a fixed price baked in at render time.
      const defaultProtein = data.pricingTable[0].protein;
      const defaultPrice = parsePrice(data.pricingTable[0].price);
        ...
        ${data.items.map(item => {
          ...
          return `
            <div class="menu-card tilt-card"
                 ...
                 aria-label="Add ${item.name} (${defaultProtein}) to cart for £${defaultPrice.toFixed(2)}"
                 onclick="addProteinItemToCart(this)"
                 onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();addProteinItemToCart(this)}">
              <div class="card-emoji">${item.emoji || '🍽'}</div>
              <div class="card-body">
                <h4>${item.name} ${tags}</h4>
                ${item.desc ? `<p>${item.desc}</p>` : ''}
                <div class="card-footer">
                  <span class="card-price" data-protein-price="${cat}">£${defaultPrice.toFixed(2)}</span>
                  <button class="add-to-cart" onclick="event.stopPropagation();addProteinItemToCart(this.closest('.menu-card'))" aria-label="Add ${item.name} to cart">+ Add</button>
                </div>
              </div>
            </div>`;
        }).join('')}
